...
#다른 데이터 셋에서 joint 수, 순서 등이 다른 걸 통일시킴
...
# 기존 bbox와 우리가 원해는 input_img 크기가 맞도록 변환 행력을 찾고 크기에 맞는 image patch를 얻어냄
# cv2.warpAffine(이미지,변환행렬,출력이미지크기)
...
# 원본 이미지의 bbox와 우리가 입력해야하는 bbox(여기선 256*192), augmentation이 주어졌을때 변환 행렬을 찾는 함수
# cv2.getAffineTransform으로 원래 이미지의 bbox(조금 회전 될 수 도 있음) -> (0,0) - (img_input_y,img_input_x)가 되도록 해주는 행렬을 찾아줌 
...
# train일 경우 image augmentation 수행 (scale, 회전, 색 변화, flip)
# 원본 이미지에서 aug등을 수행해여 inp_img에 맞는 이미지 패치로 바꿔줌
